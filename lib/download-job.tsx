            ...
            return new Promise(async (resolve) => {
                try {
                    ...
                    for (const [index, blob] of trackBlobs.entries()) {
                        const fileName = (index + 1).toString().padStart((albumTracks.length - 1).toString().length > 2 ? (albumTracks.length - 1).toString().length : 2, '0') + " " + formatTitle(albumTracks[index]) + ".flac"
                        await zipWriter.add(cleanFileName(fileName), new zip.BlobReader(blob), { signal });
                        setStatusBar(prev => ({...prev, progress: Math.floor((index + 1) / albumTracks.length * 100)}));
                    }
